...
ffmpeg_cmd = "ffmpeg -re -ar 44100 -ac 2 -acodec pcm_s16le -f s16le -ac 2 -use_wallclock_as_timestamps 1 -vf 'pad=h=(in_h+5+32), drawtext=x=(w-tw-8):y=(h-28):fontcolor=white:fontsize=28:text=%{localtime}' -i /dev/zero -f h264 -i - -vcodec copy -acodec aac -ab 128000 -g 50 -strict experimental -f flv rtmp://a.rtmp.youtube.com/live2/7ytq-b3uh-xrr5-0hmu-6x61"
