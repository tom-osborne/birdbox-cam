...
raspivid_cmd = 'raspivid -o - -t 0 -a 1036 -a "%Y-%m-%d %X" -vf -hf -fps 30 -b 6000000'
ffmpeg_cmd = "ffmpeg -re -ar 44100 -ac 2 -acodec pcm_s16le -f s16le -ac 2 -i /dev/zero -f h264 -use_wallclock_as_timestamps 1 -vf 'pad=h=(in_h+5+32), drawtext=x=(w-tw-8):y=(h-28):fontcolor=white:fontsize=28:text=%{localtime}, drawtext=x=8:y=(h-28):fontcolor=white:fontsize=28:text='birdbox', drawtext=x=32:y=(h-28):fontcolor=white:fontsize=28:text='TEST' -i - -vcodec copy -acodec aac -ab 128000 -g 50 -strict experimental -f flv rtmp://a.rtmp.youtube.com/live2/7ytq-b3uh-xrr5-0hmu-6x61"
